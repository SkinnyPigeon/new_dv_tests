...
def boilerplate(schema, engine, keys_function, hub_keys):
    metadata = MetaData()
    hub_time = Table('hub_time', metadata, schema=schema, *keys_function(hub_keys['hub_time'], 'hub_time', metadata=metadata))
    hub_person = Table('hub_person', metadata, schema=schema, *keys_function(hub_keys['hub_person'], 'hub_person', metadata=metadata))
    hub_object = Table('hub_object', metadata, schema=schema, *keys_function(hub_keys['hub_object'], 'hub_object', metadata=metadata))
    hub_location = Table('hub_location', metadata, schema=schema, *keys_function(hub_keys['hub_location'], 'hub_location', metadata=metadata))
    hub_event = Table('hub_event', metadata, schema=schema, *keys_function(hub_keys['hub_event'], 'hub_event', metadata=metadata))
    hubs = {
        'time': hub_time,
        'person': hub_person,
        'object': hub_object,
